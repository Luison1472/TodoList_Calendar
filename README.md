...
<a href="https://todolistcalendar.vercel.app/" target="_blank">**[배포된 링크](https://todolistcalendar.vercel.app/)**</a>를 통해 직접 사용해볼 수 있습니다.
...
- **배경**: 처음엔 **Intersection Observer API**를 통해 구현할 예정이었으나, 이 API는 특정 화면 지점을 기준으로 **%** 비율을 통과할 때 렌더링하는 방식.
- **문제**: 무한 캘린더의 구조에서는 과거, 현재, 미래를 계속 탐색해야 하므로, 최상단 및 최하단에 특정 포인트를 두면 무한 루프에 빠지는 문제 발견.
- **해결**: 수동으로 스크롤 이벤트를 지정하여, 최상단 + 최하단으로 스크롤할 때마다 필요한 달력만 렌더링하는 방식으로 **무한 루프** 버그를 해결했습니다.

### 2. 스크롤 튕김 현상

- **문제**: 수동 스크롤 이벤트를 통해 무한 스크롤을 구현하였으나, 스크롤이 최상단 또는 최하단으로 이동할 때마다 새로운 달력이 렌더링되면서 전체 콘텐츠의 높이가 변동.<br/>
  이로 인해 뷰포트가 위아래로 흔들리며 튕기는 현상이 발생함.
- **해결**: `useRef`로 참조된 `calendarRef`를 통해 `scrollTopBefore` 값에 `(scrollHeightAfter - scrollHeightBefore)`를 더해주어,<br/>
  변동된 스크롤 높이만큼 스크롤 위치를 조정했습니다. 이로써 자연스러운 스크롤 조정이 가능해졌습니다.
